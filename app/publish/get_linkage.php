 ...
 * 获取联动菜单接口
 */
if(($_GET['act'] != 'ajax_select')|| !$_GET['act'])  showmessage(L('error'));
...
switch($_GET['act']) {
	...
	case 'ajax_select':
		 ...
		 * 王云龙
		 */
		$parent_spell = isset($_GET['parent_spell']) ? $_GET['parent_spell'] : 'default';
		if(isset($_GET['parent_spell'])){
			if($parent_spell == "default"){
				$parent_id = 0;
			}else{
				$datas = getcache($keyid);
				$infos = $datas['data'];
				if(is_array($infos)){
					foreach($infos as $k=>$v) {
						if($v['spell'] == $parent_spell) {
							$parent_id = $v['linkageid'];
							break;
						}
					}
				}
			...
			leray_ajax_select($parent_id,$keyid);exit;
		}
		//----
 ...
 * 王云龙
 */
function leray_ajax_select($parentid,$keyid) {
	$keyid = intval($keyid);
	$datas = getcache($keyid,'linkage');
	$infos = $datas['data'];
	$json_str = "[";
	$json = array();
	foreach($infos AS $k=>$v) {
		if($v['parentid'] == $parentid && $v['spell'] != '') {
			$r = array('region_id' => $v['linkageid'],
					   'region_name' => $v['name'],
					   'region_spell' => $v['spell']);
			$json[] = JSON($r);		
